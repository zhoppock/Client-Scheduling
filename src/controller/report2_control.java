...
import database.contacts_table;
...
/**
 * This class displays the second available database report.
 ...
 */
public class report2_control extends callDirectory implements Initializable {

    public TextArea reportArea;

    /**
     * This method initializes the Second Report screen of the application,
     * which shows a report of each Contact and a sorted schedule of all of their respective appointments.
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        System.out.println("Contact Appointments Report Page Initialized");

        ObservableList<appointments> allAppointments = appointments_table.getAppointmentsSortByContactAndStart();
        reportInterface appointmentReport = n -> {
            ...
            int current_contact_id = 0;
            for(appointments appointment : n) {
                if(current_contact_id == 0) {
                    message.append(contacts_table.getContactName(appointment.getContactId())).append("'s Schedule:\n\n");
                } else if(current_contact_id != appointment.getContactId()){
                    message.append("\n--------------------------------------\n")
                            .append(contacts_table.getContactName(appointment.getContactId())).append("'s Schedule:\n\n");
                }
                ...
                message.append("APPOINTMENT ID:  ").append(appointment.getId()).append("  |  TITLE:  ").append(appointment.getTitle()).append("  |  DESCRIPTION:  ")
                        ...
                        .append(appointment.getEnd()).append("\nCUSTOMER ID:  ").append(appointment.getCustomerId()).append("\n\n");
                current_contact_id = appointment.getContactId();
            }
